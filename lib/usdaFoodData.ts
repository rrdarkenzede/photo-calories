...
export async function searchIngredientsUSDA(query: string) {
  try {
    const response = await fetch(
      `${API_ENDPOINTS.USDA_FDC}/foods/search?query=${encodeURIComponent(query)}&pageSize=10&api_key=${API_CONFIG.USDA_API_KEY}`
    );
...
export async function getUSDAFoodDetails(fdcId: string) {
  try {
    const response = await fetch(
      `${API_ENDPOINTS.USDA_FDC}/food/${fdcId}?api_key=${API_CONFIG.USDA_API_KEY}`
    );
